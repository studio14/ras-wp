{{--
  Template Name: Gifting Membership Template
--}}
...
@section('content')
  ...
  <div class="membership-pricing bg-membership-color mt-0 pt-0 xl:pt-86 pb-86 flex justify-around content-center text-center align-center items-center">
    ...
    @foreach($get_gifting_pricings as $get_gifting_pricing)
      @if ($loop->iteration%2 === 0) 
          <div class="w-full md:w-1/2 lg:w-auto md:float-right">
            <div class="pricing-item bg-white text-center items-center">
              <div class="title">
                <p  class="text-secondary">{!! $get_gifting_pricing['title'] !!}</p>
              </div>
              <div class="price-tag">
                <h2  class="text-secondary">£{!! $get_gifting_pricing['price'] !!}</h2>
              </div>
              <div class="price-desc">
                <p class="text-secondary text-left">
                    {!! $get_gifting_pricing['description'] !!}   
                </p>
              </div>
              <div class="price-button">
                <a class="btn bg-custom-bg text-secondary" href="{!! $get_gifting_pricing['page_link'] !!}" target="_blank">BUY AS GIFT</a>
              </div>
            </div>
          </div>
        @else 
          <div class="w-full md:w-1/2 lg:w-auto md:float-right">
            <div class="pricing-item bg-primary text-center items-center">
              <div class="title">
                <p  class="text-white">{!! $get_gifting_pricing['title'] !!}</p>
              </div>
              <div class="price-tag">
                <h2  class="text-white">£{!! $get_gifting_pricing['price'] !!}</h2>
              </div>
              <div class="price-desc">
              ...
              <div class="price-button">
                <a class="btn bg-custom-bg text-secondary" href="{!! $get_gifting_pricing['page_link'] !!}">BUY AS GIFT</a>
              </div>
            </div>
          </div>
        @endif
