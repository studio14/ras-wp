{{--
  Template Name: Events Calender Template
--}}
@extends('layouts.app')
            ...
            <div class="events-content">
                <div class="events-filter-bg">
                    <div class="events-filter flex flex-wrap justify-between align-center items-center">
                        <div class="events-filter-section-1 inline mb-30 flex flex-row flex-wrap justify-between">
                                <div class="input-type-bg flex-wrap">
                                    ...
                                    <select name="show_all" id="show_input" onchange="loadType(this.value, '<?= get_page_link(1054) ?>', '<?php  if (isset($_GET['date'])) echo '?date='.$_GET['date'].'&'; else echo '?' ?>category');");">
                                ...
                                <div class="input-type-bg">
                                    ...
                                    <select name="show_from" id="dateFrom" onchange="loadType(this.value, '<?= get_page_link(1054) ?>', '<?php  if (isset($_GET['category'])) echo '?category='.$_GET['category'].'&'; else echo '?' ?>date');">
                                        <option value="<?= strtotime('first day of ' . date( 'F Y')); ?>" disabled selected><?= date("F Y") ?> </option>
                                        <?php
                                            // Set current month
                                            $current_month = '';
                    ...
                    <div class="events-show grid">
                        <div class="events-show-calender">
                            <div class="container flex flex-wrap flex-row">
                            <?php
                                $paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
                                $terms = isset($_GET['category']) ? $_GET['category'] : '';
                                $args = array('post_type' => 'events', 
                                    ...
                                    'meta_query' => array(
                                        'events_date_sort' => array(
                                            ...
                                            'orderby' => 'meta_value',
                                            'order'	    => 'DESC',
                                            'value' => $_GET['date'] ? date('d/n/Y', $_GET['date']) : strtotime('first day of ' . date( 'n Y')),
                                            'compare' => '>=',
                                            'type' => 'DATE'
                                        ),
                                ...
                                $events[$postCount]['day'] = $events_date[0];
                                $events[$postCount]['month'] = $events_date[1];
                                $events[$postCount]['year'] = $events_date[2];
                                $events[$postCount]['title'] = get_the_title($query->post->ID);
                                $events[$postCount]['link'] = $events_permalink;

                                endwhile; 
                                
                               
                                //get the current date and year
                                $date = $_GET['date'] ? date('d-n-Y', $_GET['date']) : date('01-m-Y');
                                $year = date('Y', strtotime($date));
                                $month = date('n', strtotime($date));
                            ?>
                                    {!! App\draw_calendar($month, $year, $events) !!}
                            </div>
                        </div>
