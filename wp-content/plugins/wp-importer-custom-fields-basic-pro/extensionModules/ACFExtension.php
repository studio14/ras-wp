				...
				if ( ! empty( $get_acf_fields ) ) {
					...
					foreach ( $get_acf_fields as $acf_pro_fields ) {
						...
						if ( $get_field_content['type'] == 'group' ) {		
							...
							$get_sub_fields = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != 'trash' and post_parent in (%s)", array($group_field) ) );
