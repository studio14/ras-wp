...
class ScheduleExtension {
	...
	public function saveEventInformationToSchedule() {
		...
		$storeSchedule = $wpdb->insert('ultimate_cfimporter_pro_scheduled_import',
...
# Save ftp file details
					$save_ftpfile = "insert into ultimate_cfimporter_pro_ftp_schedules (schedule_id, hostname, username, password, initial_path, filename) values ('{$wpdb->insert_id}', '{$_POST['hostname']}', '{$_POST['username']}', '{$_POST['password']}', '{$_POST['initialpath']}', '{$eventKey}' , '{$_POST['portnumber']}')";
	...
	public  function smack_uci_cron_scheduler() {
		...
		$timeZone = $wpdb->get_results("select * from ultimate_cfimporter_pro_scheduled_import where isrun = 0 ");
		if(!empty($timeZone)){
			...
			$scheduleList = $wpdb->get_results("select * from ultimate_cfimporter_pro_scheduled_import where isrun = 0 and nexrun <= '$current_timestamp'");
		}
...
#$wpdb->query("update {$schedule_tableName} set imported_as = '{$returnData['eventkey']}' where id = '{$scheduledEvent->id}'");
	...
	public  function doSchedule($data,$current_timestamp) {
		...
		if($data['frequency'] != 0) {
			$wpdb->query("update ultimate_cfimporter_pro_scheduled_import set isrun = 0 where id = '{$data['id']}'");
		}
	}
}